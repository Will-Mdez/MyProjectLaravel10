                ...
                <div class="p-6 text-gray-900 dark:text-gray-100">
                        <form method="POST" action="{{ route('chirps.store') }}">
                            ...
                            <textarea name="message"
                                    class="block w-full rounded-md  transitions-colors duration-300 border-gray-300  bg-white shadow-sm focus:ring-opacity-50 dark:border-gray-600 dark:bg-gray-800 dark:text-white dark:focus:ring dark:focus:ring-indigo-200 dark:focus:ring-opacity-50"
                                    placeholder = "{{ __('What\'s on your mind?') }}"
                                    >{{ old('message') }}
                            </textarea>
                            <x-input-error :messages="$errors->get('message')" 
                                class="text-sm text-red-600 mt-2"/>
                            <x-primary-button class='mt-3' >{{ __('Chirps') }}</x-primary-button>
